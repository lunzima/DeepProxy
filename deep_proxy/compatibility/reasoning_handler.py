...
【经验性说明 — 勿按文档"优化"】
DeepSeek 官方文档称：在两个 user 消息之间，若中间 assistant 未进行工具调用，
其 reasoning_content 无需回传、传了也会被忽略。
实测不符：即便是普通 user→assistant→user 轮次，缺失 reasoning_content
同样会触发 400 "The reasoning_content in the thinking mode must be passed back
to the API."。因此本模块对所有历史 assistant 消息无差别地执行
"缓存回填 + dummy 兜底"，请勿基于文档把判定收窄到 tool-call 子链。
...
class StreamingReasoningAccumulator:
    ...
    def snapshot(self) -> Dict[int, Dict[str, Any]]:
        ...
        须**深**拷贝：槽内 tool_calls 是 list[dict]，且 merge_tool_call_deltas 会原地
        改写 inner dict（arguments += ...）；浅拷贝会让废弃尝试的 tool_call 增量污染
        snapshot，restore 后泄漏。"""
    ...
    def restore(self, snap: Dict[int, Dict[str, Any]]) -> None:
        ...
        深拷贝 snap，避免后续 consume 反向污染调用方持有的 snapshot。"""
        self._slots = copy.deepcopy(snap)
...
def ensure_reasoning_content_persistence(
    ...
    注意：实测此校验对所有历史 assistant 消息生效，不限于 tool-call 子链
