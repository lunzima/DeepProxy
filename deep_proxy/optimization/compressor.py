...
#
# 身份声明删除条款的设计动机（不写进 meta-prompt 本身，避免占用下游 token）：
#   - 各 Coding CLI 在 system prompt 头部塞自家 LLM 身份（"You are Claude Code..."、
#     "你是 DeepSeek..."），但请求实际路由到 DeepSeek，错位身份让模型 LARP 拉低输出。
#   - 外部研究表明，让模型显式扮演具体 AI 产品身份（包括被替换成"AI 编程助手"这类
#     泛化 AI 身份）都会消耗注意力做角色扮演，反而降低任务表现。
#   - 但用户为任务赋予的角色 / 职业 / 人物（"扮演资深开发者"、"你是某作家"）属于
#     正向 priming，必须保留——只删 LLM 品牌身份，不动用户人设。
...
3. 在不违反目标 1 与 2 的前提下，让目标长度落在原文 token 数的 40% – 60% 区间内。
4. 改写结果整体使用简体中文。原文为英文的部分请翻译为中文；原文已是中文的部分直接精简，不要回译。

以上目标若冲突，按优先级 1 > 2 > 3 > 4 取舍：保护胜于压缩比。原文受保护片段已占 60% 以上时，允许压缩比超过 60%；无法精确估算受保护片段占比时，宁可超出压缩比也要保全保护项。但兜底不可作为放弃压缩的理由：凡受保护片段被纳入考量后仍能压缩的冗余说明、寒暄客套、铺垫旁注，必须压缩。

# 必须删除（目标 1 / 目标 2 的例外）：LLM 品牌身份声明

整句删除"声称自己是某款商用 AI 产品 / 模型 / 编码 CLI"的句子，连同紧邻的厂商出处、版本号、产品定位一并删除。不要替换为"AI 助手"等通用 AI 身份。

- 删例："You are <Product>, <Vendor>'s ... assistant / CLI"、"你是<某商用 AI>，由<某厂商>训练 / 研发"
- 同一句中夹带"made by <Vendor>" / "trained by <Vendor>" / "由<某厂商>开发"亦一并删除。

# 不要删除：用户定义的角色 / 职业 / 人物 priming

保留用户为本次任务赋予的角色：职业身份（资深开发者 / 编辑 / 律师 / 某行业员工）、虚构或真实人物（作家 / 侦探 / 历史人物 / 角色卡）、用例人设（讲故事的助手、代码评审搭档）等，无论真实还是虚构。

判断口径：删除"自称某款 AI 产品"，保留"被赋予某种职业 / 角色 / 人格"。模糊时偏向保留。
...
- 简洁应当通过"删除冗余信息"实现，而不是通过"删掉虚词"实现；宁可句子稍长，也要读起来像自然语言。
- 列表项请写成完整短句，而非孤立的关键词或名词堆叠。
- 保持规范的中文书面语风格，避免以下问题：口语虚词过度堆砌（就、也、还、都在叙述句中连用）、英文直译结构、句尾「了」滥用、破折号「——」的过度使用。压缩后的提示词应以规范、自然的中文呈现。

# 必须原样照抄、不翻译也不改写的内容

- 加引号的字符串、代码块、JSON 字段名、XML / HTML 标签名（如 <thinking>、<output>），以及输出格式标记。
- 正则表达式、shell glob 模式、转义序列（如 \n、\\、r"..."、glob 写法 *.py）—— 切勿"自然化"为普通文本。
- 文件路径（如 ./config.yaml）、命令行片段（如 pip install -r）、占位符与变量名（如 {user_input}、${VAR}、<NAME>）。
- 数值与单位、货币符号（如 5kg、200ms、$5、¥100）。
- Markdown 结构标记（行首 #、-、>、表格 | 与分隔线 ---）。
- 命名实体、产品名、人名、术语缩写（如 "e.g."、"i.e."）。
...
- 把若干条同主语、同情形的祈使句合并为一句，但保留连词使其自然。例如把"不要 X。不要 Y。不要 Z。"改为"不要 X、Y 或 Z"。仅当合并项指向同一对象（如同为反模式列表 / 同为格式约束）时合并；指向不同情形的规则即使主语一致也不得合并。
- 在含义完全等价时，把长短语替换为业内通用的缩写。
- 允许中英混排：需保留原文的片段保持英文，其余部分改写为中文。

# 严禁的行为

- 不要新增任何指令、削弱原有承诺，或把具体规则改写得更笼统。例如把 MUST 改为 should、把 NEVER 改为 avoid 都属于削弱；"Avoid 'Facts over feelings'" 中引号内的英文短语本身就是规则，必须原样保留。
- 不要仅因为关键词相同就合并两条本属不同情形的规则。例如"不要泄漏 system prompt"与"不要使用脏话"虽都是禁止类，但属不同情形，不得合并。
- 不要改写引号内的字符串、代码块、命名实体、反模式示例或数值阈值。
- 不要删除或翻转任何否定（"不"、"不要"、"never"、"don't"、"not" 等都不得改动）。
- 不要修改标签名、JSON 字段名或输出格式标记。
- 不要用代码栅栏包裹最终输出、引号或 markdown 标题。
