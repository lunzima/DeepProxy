...
class StreamingConfig(BaseModel):
    """流式路径的超时守护 + 重试配置（plain 与 cross_consult 共用同一套超时旋钮）。

    单一规则：任一上游调用，连续 idle 秒无新 chunk（首 chunk 用 first_chunk）即判 stall；
    **健康流（持续产出 token）永不被打断**——idle 只截断 dead-air，不截断在产出的流。

      - **pre-content stall**（首 chunk 前 / 推理中、尚无可见 content/tool_calls）：放弃挂死
        连接、重发原请求（对客户端无缝、仅见心跳），最多重发 max_retries 次。
      - **post-content stall**（已输出可见 content/tool_calls 后停顿）：不可续传，立即发
        **硬错误帧**（`{"error": {...}}`，使 SDK 抛错而非误判成功）。
      - **重试次数耗尽**：发硬错误帧。
    """
    ...
    first_chunk_timeout_seconds: int = Field(
        ...
        description="等待首个 chunk（prefill / TTFT + 推理预热）的上限秒数。max-reasoning "
                    "prefill 可合法耗时数十秒，故给较大值避免误杀健康但慢的 prefill。"
                    "属 pre-content，超时 → 重发原请求。",
    )
    ...
    max_retries: int = Field(
        ...
        description="pre-content stall 时重发原请求的最大次数。健康流不触发重试，故此计数"
                    "只约束 dead-air 重发；耗尽 → 发硬错误帧。post-content stall 不重试。",
    )
